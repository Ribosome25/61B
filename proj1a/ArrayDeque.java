public class ArrayDeque<T> {
    /** add and remove must take constant time, except during resizing operations.
     ...
     For example, if you add 10,000 items to the deque, and then remove 9,999 items, you shouldn’t still be using
     an array of length 10,000ish. For arrays of length 16 or more, your usage factor should always be at least 25%.
    ...
    /* next index update:
        ...
        if decrease, should consider the case that it went below 0.
        (i+len-1)%len should work.
     */
    private int nextFirst;
    private int nextLast;

    public int increaseFactor = 2;
    ...
    public void increaseSize(){
        ...
        // Left side: from 0 to 0, length is the id of nextLast. The id of next Last does not change.
        System.arraycopy(this.items, 0, expandItems, 0, this.nextLast);
        // right side: From First+1, (some times may from 0), copy the rest. The id of next First change to len2-len1+F
        System.arraycopy(this.items, (this.nextFirst+1)%this.items.length, expandItems,
                len2 - len1 + 1 + this.nextFirst, len1 -1 - this.nextFirst);
